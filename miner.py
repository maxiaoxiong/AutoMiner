...
parser.add_argument('--server', type=str, default=None, help='Send succ info to server')
...
def submit_block(key):
    ...
    if found_valid_hash:
        ...
        if(retries > max_retries):
            print(f"Failed to submit block after {retries} retries")
            return None
        # upload success info to custom server
        if server:
            send_succ_info_to_custom_server(submitaccount, worker_id, hashed_data, key)
        return key, hashed_data
    return None


def send_succ_info_to_custom_server(account, worker_id, hashed_data, key):
    payload = {
        "worker": worker_id,
        "key": key,
